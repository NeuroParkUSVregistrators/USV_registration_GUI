...
const SCROLL_MASK = (1 << 21) | (1 << 23) # GDK_SCROLL_MASK | GDK_SMOOTH_SCROLL_MASK
...
include(joinpath(@__DIR__, "Necessities", "usv_detection_corefunctions_temp.jl"))
...
function make_gui()
    try
        ...
        function update_window_title!(
            ...
            file_label = isempty(filename) ? "USV Manual" : basename(filename)
            Gtk.GAccessor.title(win, dirty ? "$(file_label) *" : file_label)
            return nothing
        end
        ...
        function load_into_gui!(nm::AbstractString) # seperation : 2026-03-04 
            ...
            if hasproperty(spec, :t_sng) && length(spec.t_sng) >= 2
                ...
                t_sng_obs[] = range(0.0, step=dt_sng_obs[], length=size(spec.fout, 2))
            end
        ...
        (nm === nothing || isempty(nm)) && return nothing
        load_into_gui!(nm)

        # glade 
        b = Gtk.Builder(filename=joinpath(@__DIR__, "usv_vlaptop.glade"))

        # whole window 
        win = b["win"]

        function set_dirty!(value::Bool)
            dirty_ref[] == value && return nothing
            dirty_ref[] = value
            update_window_title!(win, nm_ref[]; dirty=value)
            return nothing
        end

        update_window_title!(win, nm_ref[]; dirty=false)
        ...
        function make_canvas_responsive!(cnvs)
            w = unwrap_widget(cnvs)
            try
                Gtk.set_gtk_property!(w, :hexpand, true)
                Gtk.set_gtk_property!(w, :vexpand, true)
                # No height_request: allow the window to shrink vertically.
            catch e
                @warn "Could not set responsive canvas properties" exception=(e, catch_backtrace())
            end
            return cnvs
        end

        # Make GtkObservables sliders actually fill the vertical allocation
        function make_vertical_slider_responsive!(s)
            w = unwrap_widget(s)
            try
                Gtk.set_gtk_property!(w, :hexpand, false)
                Gtk.set_gtk_property!(w, :vexpand, true)
                # GtkAlign.FILL == 0 in GTK3; using integer avoids Gtk.jl enum-version differences.
                Gtk.set_gtk_property!(w, :valign, 0)
            catch e
                @warn "Could not set vertical slider properties" exception=(e, catch_backtrace())
            end
            return s
        end
        ...
        singlew = unwrap_widget(singleusvcanvas)
        Gtk.set_gtk_property!(singlew, :hexpand, true)
        Gtk.set_gtk_property!(singlew, :vexpand, false)
        Gtk.set_gtk_property!(singlew, :height_request, 120)
        ...
        if isempty(liness[]) # if there is no usv 
            ...
            smax = max(numusv, 2) # slider needs min < max
            singleslider = slider(1:smax; observable=usvboxind)
            singlespin = spinbutton(1:smax; observable=usvboxind)
        end
        ...
        # Histogram panel should expand horizontally, but must not steal vertical space
        Gtk.set_gtk_property!(usvcanvas, :hexpand, true)
        Gtk.set_gtk_property!(usvcanvas, :vexpand, false)
        Gtk.set_gtk_property!(usvcanvas, :height_request, 200)
        ...
        # Air-flow panel removed: spectrogram now uses the upper display area.
        ...
        timespan_w = unwrap_widget(timespan)
        Gtk.set_gtk_property!(timespan_w, :hexpand, true)
        Gtk.set_gtk_property!(timespan_w, :vexpand, false)
        Gtk.set_gtk_property!(timespan_w, :valign, 3)  # GTK_ALIGN_CENTER
        push!(timespanbox, timespan)

        # time scale slider 
        timescale = slider(0.1:0.1:15.0)
        timescale_w = unwrap_widget(timescale)
        Gtk.set_gtk_property!(timescale_w, :hexpand, true)
        Gtk.set_gtk_property!(timescale_w, :vexpand, false)
        Gtk.set_gtk_property!(timescale_w, :valign, 3)
        push!(timescalebox, timescale)

        # time spin button 
        timespin = spinbutton(1:100:(npixel-spanpixel); observable=timespan.observable)
        timespin_w = unwrap_widget(timespin)
        Gtk.set_gtk_property!(timespin_w, :vexpand, false)
        Gtk.set_gtk_property!(timespin_w, :valign, 3)
        ...
        sngscale_w = unwrap_widget(sngscale)
        Gtk.set_gtk_property!(sngscale_w, :vexpand, true)
        Gtk.set_gtk_property!(sngscale_w, :valign, 0) # GTK_ALIGN_FILL
        ...
        make_canvas_responsive!(sngcanvas)
        ...
        function do_detection()
            safe_cb("automatic USV detection") do
                ...
                println("Automatic USV detection starts...")

                # 중요: 
                # spec.fout 자체를 직접 넘기면 specdiscont() 과정에서 
                # normalization 때문에 원본 spectrogram을 mutate할 가능성이 있음. 
                # 반드시 copy 사용. 
                sng = copy(spec.fout)

                # 1. denoise 
                sng_denoised = denoise_usv(sng, spec.f)

                # 2. detect (시간축 정확도 향상: amplitude/SNR gate + confident false-positive) 
                ON, OFF = usv_whole_2d(
                    sng_denoised,
                    spec.dt_sng,
                    spec.t_sng,
                    spec.f;
                    use_amp_gate=false,   # true 로 켜면 정밀도↑ 이지만 조용한 콜/가장자리 잘림 
                    fp_mode=:confident,   # :confident(고립 콜은 강하면 유지) | :original(고립 전부 삭제) | :none(FP 끔) 
                    extend=true,          # 연속 syllable 의 조용한 onset/offset 까지 확장 
                    snr_ext=1.2,          # 경계 확장 임계 (낮출수록 더 확장) 
                    sp_lo=0.15,           # 확장 시 최소 spectral purity 
                    verbose=true          # 단계별 검출 개수 콘솔 출력 (디버그) 
                )

                println("$(length(ON)) USVs detected.")

                # 3. GUI box로 변환 (세로 방향 탐지: syllable별 실제 주파수 대역 추정) 
                detected = detection_to_liness_2d(
                    ON,
                    OFF,
                    spec,
                    sng_denoised;
                    flow=15_000.0,
                    fhigh=110_000.0,
                    select=:local,        # 컬럼별 local SNR 로 주파수 대역 산정 (스윕 편향 방지) 
                    local_k=2.0,          # 낮출수록 조용한 쪽 주파수까지 포착 (잡음 위험↑) 
                    margin_bins=4,        # 대역 상/하 여유 (bin, 1 bin ≈ 0.49 kHz) 
                    min_height_bins=6     # box 최소 높이 (feature 계산 안정용) 
                )
                ...
                gui_idle("after automatic detection") do
                    request_liness_notify()
                    request_hist_update()
                    Observables.notify(usvboxind)
                end

                println("Detection result loaded into GUI.")
            end
        end
        ...
        # detection button
        detectioncb = b["detection"]
        signal_connect(detectioncb, "clicked") do _
            do_detection()
        end
        ...
        Gtk.add_events(sngw, SCROLL_MASK)
        signal_connect(sngw, "scroll-event") do _, event
        ...
        onany(usvboxind, obs) do current, total
            gui_idle("usvboxind/obs changed") do
                ...
                if cctime < currtime[] || cctime > currtime[] + timescale.observable[]
                    ...
                    npixel = size(fout_obs[], 2)
                    spixel = max(1, Int(floor(timescale.observable[] / dt_sng_obs[])))
        ...
        sigextend = on(sngcanvas.mouse.motion) do btn
            try
                safe_cb("mouse motion") do
                    ...
                    if moving[]
                        bb = mov(btn, currtime, timescale, click_pivot)
                    ...
                    if modifying[]
                        liness[][usvboxind[]][2] =
                            unit_to_time(btn.position, currtime[], timescale.observable[])
                        set_dirty!(true)
                        request_liness_notify()
                    end
                end
        ...
        Gtk.showall(win)
        update_window_title!(win, nm_ref[]; dirty=dirty_ref[])

        # Canvas redraw is managed by GtkObservables draw callbacks.
        nothing
    catch e
...
function three(spec, lns)
    try
        ...
        # Compact responsive layout: 3 plots on top, 2 below.
        # Five axes in one row makes tick/x-label text overlap in narrow windows.
        ...
        ax1.yticklabelsize = 9
        # ax1.xticks = ([0,150],["0","150"]) 
        # ax1.yticks =([0.0,0.1],["0","0.1"]) 
        ax2.xlabel = "loudness (dB)"
        ax2.xlabelsize = 11
        ax2.xticklabelsize = 9
        ax2.yticklabelsize = 9
        # ax2.xticks = ([-10,0,20],["-10","0","20"]) 
        ax3.xlabel = "spectral purity"
        ax3.xlabelsize = 11
        ax3.xticklabelsize = 9
        ax3.yticklabelsize = 9
        ax4.xlabel = "mean freq. (kHz)"
        ax4.xlabelsize = 11
        ax4.xticklabelsize = 9
        ax4.yticklabelsize = 9
        # ax4.xticks =([50,100],["50","100"]) 
        ax5.xlabel = "pitch variance (log₁₀ Hz²)"
        ax5.xlabelsize = 11
        ax5.xticklabelsize = 9
        ax5.yticklabelsize = 9
...
function update_sng(fout::AbstractMatrix{<:Real}, ts::Int, tw::Real, sr::Real; dt_sng::Real=0.000512)
    ...
    spixel = max(1, Int(floor(tw / dt_sng)))
...
update_sng(
    fout::AbstractMatrix{<:Real},
    ts::Int,
    tw::Real,
    sr::Real,
    dt_sng::Real;
    kwargs...,
) = update_sng(fout, ts, tw, sr; dt_sng=dt_sng)
